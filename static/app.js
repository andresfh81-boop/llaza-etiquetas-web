...
const EXTRAS = [
  ...
  { nombre: 'TRANSF. LED PERIMETRAL', activo: false },
  { nombre: 'TRANSF. LED LAMA', activo: false },
  { nombre: 'TRANSF. FOCO', activo: false },
];
...
function regeneraAuto() {
  ...
  if (autoTapa) {
    for (const { v, mod } of base) {
      ...
      if (m) anadirFila(trad('T. SUP.') + ' ' + m[1], true, mod, 'TAPA SUP. ' + m[1]);
    }
  }
